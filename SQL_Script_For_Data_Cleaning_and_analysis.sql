...
-- solving queries related to this
--      A)Roll metrics
...
-- 6)how many,each type of roll is delivered
...
-- 9) max_no_of_orders_made_by_each_user_and_those_are_delivered_successfully
...
-- 9) max no of rolls which are delivered in single order..(ie;on sme order_id how many rolls are ordered)
...
-- #o...you can change it to int,,but our use case is string,,bcz we are mentioning list of values separated with comma ,,so we keep varchar only

-- a) Modifying Actual Dataset Using UPDATE clause 
-- b) Creating VIEW to access anywhere in Current session
-- c) Use WITH clause for creating temporary updated table for accessing throught  scope of query itself
...
-- i think better option is 3rd one...one
            ...
            -- b) Use WITH Clause functionality
...
-- 11) for each customer , how many delivered rolls had at least 1 change and how many had no changes?
            with temp_customer_orders (order_id,customer_id , roll_id , not_include_items , extra_items_included , order_date) as
                    (
                        select order_id , customer_id , roll_id ,
                                                                    case 
...
--  12) total changes made by each user out of all orders thaey made in past
            with temp_customer_orders (order_id,customer_id , roll_id , not_include_items , extra_items_included , order_date) as
                    (
                        select order_id , customer_id , roll_id ,
                                                                    case 
...
-- 13)How many rolls were delivered that had both exclusion and extras?
                select * 
                from
                (
...
-- 14) what was the total no of rolls ordered for each hour of the day?
...
-- 15) what was the no of orders "in_total", for each day of the week ...?
...
-- 16) what was the avg time in minutes it took for each  driver  to arrive  at the fassos HQ(head quarter) to pickup the order?	
...
-- 17) is there any relation ship between no of rolls and how long the order takes to prepare
            ...
            -- see relationship_between_rollcount_and_avg_time_to_prepare_order.xlsx file
            -- Result From Excel we get............
            -- As u see,,there exist a Linear Relationship Between roll_count and avg_time_to_prepare_order,,,ie;if no of rolls increase in order then ,, time to prepare that order is also incrased								increased,,,(Positive Correlation)….and avg time to preapre 1 roll is around around 10 min


-- 18) what was the average distance travelled for each customer?
...
-- 19) what was the difference bn longest and shortest delivery times for all orders?
...
-- 20) Max duration time taken and minimum duration time taken to deliver order in past
...
-- 21) Order details of Max duration time taken and minimum duration time taken to deliver order in past
                select *
                from
                (
...
-- 22) what was the avg speed for each driver for each delivery and do you notice any trend for these values?
...
-- 23) what is successful order delivery percentage for each driver?
...
-- 24) Most used ingredients for each roll:
